...
public class UpdaterNEW {
    ...
    private static final String QUERY = "/servermods/files.php?projectIds="; // Path to GET
    private static final String HOST = "http://instancelabs.eu5.org"; // Slugs will be appended to this to get to the project's RSS feed

    private static final String[] NO_UPDATE_TAG = { "-DEV", "-PRE", "-SNAPSHOT" }; // If the version number contains one of these, don't update.
    private static final int BYTE_SIZE = 1024; // Used for downloading files
    private YamlConfiguration config; // Config file
    private String updateFolder;// The folder that downloads will be placed in
    private UpdaterNEW.UpdateResult result = UpdaterNEW.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
     ...
     */
    public UpdaterNEW(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(UpdaterNEW.HOST + UpdaterNEW.QUERY + id);
     ...
     */
    public UpdaterNEW.UpdateResult getResult() {
        this.waitForThread();
        return this.result;
    }
    ...
    private void saveFile(File folder, String file, String u) {
        ...
        try {
            ...
            final byte[] data = new byte[UpdaterNEW.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, UpdaterNEW.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = UpdaterNEW.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    int b;
                    final byte buffer[] = new byte[UpdaterNEW.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, UpdaterNEW.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, UpdaterNEW.BYTE_SIZE)) != -1) {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(" v").length == 2) {
                ...
                if (this.hasTag(version) || version.equalsIgnoreCase(remoteVersion) || (curVer >= remVer)) {
                    ...
                    this.result = UpdaterNEW.UpdateResult.NO_UPDATE;
                    return false;
                }
            } else {
                ...
                this.result = UpdaterNEW.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean hasTag(String version) {
        for (final String string : UpdaterNEW.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdaterNEW.TITLE_VALUE);
            this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdaterNEW.LINK_VALUE);
            this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdaterNEW.TYPE_VALUE);
            this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdaterNEW.VERSION_VALUE);
    ...
    private class UpdateRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (UpdaterNEW.this.url != null) {
                // Obtain the results of the project's file feed
                if (UpdaterNEW.this.read()) {
                    if (UpdaterNEW.this.versionCheck(UpdaterNEW.this.versionName)) {
                        if ((UpdaterNEW.this.versionLink != null) && (UpdaterNEW.this.type != UpdateType.NO_DOWNLOAD)) {
                            String name = UpdaterNEW.this.file.getName();
                            // If it's a zip file, it shouldn't be downloaded as the plugin's name
                            if (UpdaterNEW.this.versionLink.endsWith(".zip")) {
                                final String[] split = UpdaterNEW.this.versionLink.split("/");
                                name = split[split.length - 1];
                            }
                            UpdaterNEW.this.saveFile(new File(UpdaterNEW.this.plugin.getDataFolder().getParent(), UpdaterNEW.this.updateFolder), name, UpdaterNEW.this.versionLink);
                        } else {
                        	UpdaterNEW.this.result = UpdateResult.UPDATE_AVAILABLE;
                        }
                    }
                }
            }
